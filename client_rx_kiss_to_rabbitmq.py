...
# Authoer:	   Lori
# Date:	       April 2018
# Description: Script for receiving KISS hex from TCP connection and creating RabbitMQ messages
################################################################
        ...
        if data:
            ...
            if kiss_frame[0] == FEND and kiss_frame[-1]==FEND: #KISS Frame Detected
                ...
                try:
                    ...
                    ax25_frames_json = json.dumps(ax25, indent=4)
                    print "Trying now: "
                    print ax25_frames_json
                    
                    # Publish frame to RabbitMQ
                    key = "kiss_hex_key"
                    channel.basic_publish(exchange = exc, routing_key = key, body = ax25_frames_json)
                    print datetime.datetime.now().time(), "\tPublished message with routing_key: '%s'" % key
                    print "\r\n\r\n\r\n"
